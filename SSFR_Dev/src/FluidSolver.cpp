...
void FluidSolver::loadFile() {
	...
	*/
	...
	//Json
	ofxJSONElement json;
	string path = "data02.json";
	bool success = json.open(path);

	if (success) {
		int particleNum = json["Position"].size();
		cout << "Particle Num : " << particleNum  << endl;
		for (int i = 0; i < particleNum; i+=3) {
			ofVec3f p = ofVec3f(json["Position"][i].asFloat(), json["Position"][i + 1].asFloat(), json["Position"][i + 2].asFloat());
			//cout << p << endl;
			pos.push_back(p);
			particle.addVertex(p);
		}
